...
if [ "$TARGET" = "claude" ]; then
  ...
  # Note: settings.local.json is machine-specific and should not be committed
  if [ -f "${SCRIPT_DIR}/settings.local.json" ] && [ ! -e "${CLAUDE_DIR}/settings.local.json" ]; then
  ...
  if [ -d "${SCRIPT_DIR}/skills/theologian" ]; then
    echo ""
